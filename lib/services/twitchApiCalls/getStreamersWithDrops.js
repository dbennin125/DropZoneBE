...
function getVerifiedDrops(streamerName) {
  return request.post('https://gql.twitch.tv/gql')
    ...
    .then(([{ data }]) => ({
      ...
      streamDropsInfo: data.user.stream.isStreamDropsEnabled,
...
function userHasDrops(userData) {
  return userData.user.lastBroadcast.game?.activeDropCampaigns
    .some(campaign => campaign.isAvailableToAllChannels || campaign.applicableChannels
      .find(channel => channel.id === userData.user.id));
}
